    ...
    domain.run(function () {
      ...
      middleware = function expressRedisCache_Middleware (req, res, next) {
        ...
        self.get(name, domain.bind(function (error, cache) {
          ...
          if ( cache.length ) {
            console.log('cached');
            console.log(cache[0].body, cache[0].type);
            res.contentType(cache[0].type || "text/html");

            // if( cache[0].type.indexOf('application/json') >= 0 ) {
          ...
          /** otherwise, cache request **/
            ...
            res.send = function (body) {
              var type = 'application/json'; //res.get('Content-Type');
              var isJSON = true; //type && type.indexOf('application/json') >= 0;
